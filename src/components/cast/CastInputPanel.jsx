...
export default function CastInputPanel({ onCast }) {
  ...
  return (
    <section className="panel hero-panel">
      <p className="eyebrow">The Emergent Ones</p>
      <h1 className="hero-title">Eidomancer</h1>
      <p className="hero-copy">
        Ask a real question. Receive a symbolic cast. A modern, adaptive system for navigating uncertainty
        through pattern, tension, and insight.
      </p>
      ...
      <textarea
        ...
        onChange={(event) =>
          dispatch({ type: actionTypes.SET_INPUT_QUESTION, payload: event.target.value })
        }
      ...
      <div className="prompt-group">
        {suggestedPrompts.map((prompt) => (
          <button
            ...
            onClick={() => dispatch({ type: actionTypes.SET_INPUT_QUESTION, payload: prompt })}
          >
            {prompt}
          </button>
        ))}
