...
const phase = state && state.status ? state.status.phase : "idle";
const message =
state && state.status ? state.status.message : "Ask a question to begin.";
const error = state && state.status ? state.status.error : "";

return (
<div className={"status-banner status-" + phase}> <strong>{message}</strong>
{error ? <p className="status-error">{error}</p> : null} </div>
);
}

export default function CastResultPanel() {
const { state } = useEidomancer();
const cast = state ? state.activeCast : null;
const rawResponse = state ? state.rawResponse : null;

return ( <section className="panel result-panel"> <StatusBanner />
