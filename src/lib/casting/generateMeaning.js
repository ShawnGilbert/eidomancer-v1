function buildFallbackCast(question) {
  return {
    ...
    question,
    pattern: "You are standing inside a repeated friction between what matters and what is easiest to postpone.",
    tension: "Clarity is present, but momentum is leaking through hesitation, over-processing, or split attention.",
    insight: "The pattern is not confusion alone. It is partial knowledge without committed structure.",
    advice: "Reduce the question to one actionable move. Complete one visible step before asking the system for a deeper answer.",
...
export async function generateMeaning(request) {
  ...
  const hasStructuredContent =
    ...
    [parsed.title, parsed.pattern, parsed.tension, parsed.insight, parsed.advice, parsed.echo].some(
